...
public class BasicIntegrationTest extends TestVerticle
{
  ...
  public void testHTTP()
  {
    ...
    vertx.createHttpServer().requestHandler(req -> req.response().end()).listen(8181, asyncResult ->
    {
      ...
      vertx.createHttpClient().setPort(8181).getNow("/", resp ->
      {
        ...
        /*
         * If we get here, the test is complete You must always call `testComplete()` at the end. Remember that
         * testing is *asynchronous* so we cannot assume the test is complete by the time the test method has
  ...
  @Test
  /*
   * This test deploys some arbitrary verticle - note that the call to testComplete() is inside the Verticle
   * `SomeVerticle`
   */
